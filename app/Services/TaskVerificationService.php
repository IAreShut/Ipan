...
class TaskVerificationService
{
     ...
     * @return void
     */
    public static function evaluateStudentTasks(User $student): void
    {
        try {
            ...
            $maxTargetWeek = $pendingTasks->max('target_week');

            // 3. Fetch student's log entries (pending or approved) up to max target week in 1 single optimized query, grouped by week_number
            $validStatuses = ['pending', 'approved'];
            
            $logEntries = LogEntry::where('student_id', $student->id)
                ->whereIn('status', $validStatuses)
                ->where('week_number', '<=', $maxTargetWeek)
            ...
            $totalWeeks = optional($internship)->total_weeks ?? 12;
            ...
            Log::error('Task verification failed: ' . $e->getMessage());
        }
    }
}
