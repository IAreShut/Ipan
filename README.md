...
├── public/
│   ├── css/                               # Page-specific stylesheets (16 files)
...
| `class` | TEXT | NULLABLE | JSON array via accessor |
| `programme_code` | TEXT | NULLABLE | JSON array via accessor |
...
### MVC with Controller-per-Role Pattern
...
### Dual-Path File Storage Pattern
...
### Phone Normalisation (Malaysian +60)
...
### Pre-Assigned Supervisor Matching

Registration uses a lookup table (`supervisor_assignments`) instead of string matching:
...
`FIELD()` is MySQL-specific. Will NOT work on SQLite.

### Internship Auto-Creation
...
### Log Entry Creation & AI Summary Flow
...
### Registration & Pre-Assigned Supervisor Flow
...
### Notification Delivery Flow (Scheduled + Real-Time)
